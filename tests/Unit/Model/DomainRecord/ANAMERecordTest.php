...
class ANAMERecordTest extends RecordTestCase
{
    // We only care about the record type
    public function testApiDataParsedCorrectly(): void
    {
        $this->mock->append(new Response(200, [], $this->getFixture('responses/domainrecord/aname.json')));
        $record = $this->domain->records->get(732673);
        $this->assertEquals(RecordType::ANAME(), $record->type);
    ...
    public function testDataSentToApiCorrectly(): void
    {
        ...
        $this->assertJsonStringEqualsJsonString($this->getFixture('requests/domainrecord/create-aname.json'), $history[0]['request']->getBody());
    }
}
